...
RUSSIAN_MESSAGE_X = CANVAS_WIDTH / 2
...
class RussianWordleGWindow:
    ...
    def __init__(self):
        ...
        def create_keyboard():
            keys = { }
            nk = len(RUSSIAN_KEY_LABELS[0])
            h = KEY_HEIGHT
            y0 = CANVAS_HEIGHT - BOTTOM_MARGIN - 3 * KEY_HEIGHT - 2 * KEY_YSEP
            for row in range(len(RUSSIAN_KEY_LABELS)):
                y = y0 + row * (KEY_HEIGHT + KEY_YSEP)
                x = (CANVAS_WIDTH - nk * RUSSIAN_KEY_WIDTH - (nk - 1) * KEY_XSEP) / 2
                if row == 1:
                    x += (RUSSIAN_KEY_WIDTH + KEY_XSEP) / 2
                for col in range(len(RUSSIAN_KEY_LABELS[row])):
                    label = RUSSIAN_KEY_LABELS[row][col]
                    w = RUSSIAN_KEY_WIDTH
                    if len(label) > 1:
                        w += (RUSSIAN_KEY_WIDTH + KEY_XSEP) / 2
                    keys[label] = WordleKey(self._canvas, x, y, w, h, label)
                    x += w + KEY_XSEP
            return keys
